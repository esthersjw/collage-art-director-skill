...
Request a text-free base with a planned typography path. Never ask an image model to render exact required wording. The path may cross the photo or a subject's non-critical area; protect eyes, mouth, hands, and the key gesture rather than isolating words in generic empty panels.
